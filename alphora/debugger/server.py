...
#
# from alphora.debugger.frontend import get_html
#
#
...
# ==========================================
# 新增：内置的前端资源打包函数
# 替代原本的 from .frontend import get_html
# ==========================================
def get_bundled_html():
    ...
    # 3. 定义需要注入的文件列表 (顺序必须与 index.html 中的引用顺序一致)
    css_files = [
    ...
    for js_file in js_files:
        ...
        if os.path.exists(file_path):
            with open(file_path, 'r', encoding='utf-8') as f:
                js_content = f.read()
            # 将 <script src="..."> 替换为 <script>...</script>
...
def start_server_background(port: int = 9527):
    ...
    def run():
        from .tracer import tracer
        # 注意：这里移除了原本的 from .frontend import get_html
