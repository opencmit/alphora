...
"""
    ...
    def _get_sandbox_prompt() -> str:
        return (
            ...
            "```\n"
            "/mnt/workspace/          ← working directory (cwd, read-write)\n"
            "/mnt/uploads/            ← user-uploaded files (read-only)\n"
            "/mnt/outputs/            ← final output files for the user (read-write)\n"
            "/mnt/skills/             ← skill scripts & resources (read-only)\n"
            "```\n\n"
            "### Important rules\n\n"
            "1. **Check uploads first**: Before starting any task, run "
            "`list_files` on `/mnt/uploads/` or `ls /mnt/uploads/` to see what "
            ...
            "3. **Write final outputs to `/mnt/outputs/`**: Any file the user "
            ...
            "5. **`/mnt/uploads/` and `/mnt/skills/` are read-only**: Do NOT "
            "attempt to write or modify files in these directories.\n"
        )
